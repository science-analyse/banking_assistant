...
Uses local embeddings (sentence-transformers) and Gemini for LLM only
"""
...
class BankCardRAG:
    """RAG system for Bank of Baku card information with local embeddings"""
    ...
    def __init__(self, data_file: str = "data/rag_chunks.jsonl"):
        ...
        # Use local multilingual embedding model (works offline, free)
    ...
    def load_and_index_data(self):
        ...
        # Add to ChromaDB (it will generate embeddings automatically using local model)
        print("Generating embeddings locally (this is free and offline)...")

        # Process all at once (local model is fast)
        self.collection.add(
            documents=documents,
            metadatas=metadatas,
            ids=ids
...
def main():
    """Example usage"""
    print("=" * 60)
    print("Bank of Baku RAG System (Local Embeddings)")
    print("=" * 60)
